...
public sealed class TrayStartBreakCommandHandler(
    ITrayCurrentDevice device,
    IAttendanceTodayStateService todayState,
    StartBreakCommandHandler inner,
    ITenantRepository tenants,
    ITenantContextSwitcher tenantSwitcher)
    : IRequestHandler<TrayStartBreakCommand, Result<AttendanceTodayResponse>>
{
    public async Task<Result<AttendanceTodayResponse>> Handle(
        TrayStartBreakCommand request, CancellationToken ct)
    {
        ...
        return await inner.HandleForContextAsync(contextResult.Value!, autoDetected: true, ct);
    }
}

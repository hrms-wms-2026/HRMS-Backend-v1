...
public sealed class TrayEndBreakCommandHandler(
    ITrayCurrentDevice device,
    IAttendanceTodayStateService todayState,
    EndBreakCommandHandler inner,
    ITenantRepository tenants,
    ITenantContextSwitcher tenantSwitcher)
    : IRequestHandler<TrayEndBreakCommand, Result<AttendanceTodayResponse>>
{
    public async Task<Result<AttendanceTodayResponse>> Handle(
        TrayEndBreakCommand request, CancellationToken ct)
    {

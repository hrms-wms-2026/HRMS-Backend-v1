    ...
    : IRequestHandler<RemoveEventMeetingCommand, Result>
{
    public async Task<Result> Handle(RemoveEventMeetingCommand request, CancellationToken ct)
    {
        ...
        if (meeting is null)
        {
            // No calendar_event_meetings row exists - e.g. the user manually pasted a Zoom/Teams
            // URL into the meeting-link field rather than auto-generating one via
            // CreateEventMeetingCommand. The frontend's isAutoGeneratedMeetingLink() heuristic still
            // shows a "Remove" button for any zoom.us/teams.microsoft.com URL and optimistically
            // clears its local state on click, so MeetingLink must be cleared here too (there is
            // nothing to cancel remotely, since no meeting row/connection is tracked) or the link
            // silently reappears on next load once backend and frontend state disagree.
            if (existing.MeetingLink is not null)
            {

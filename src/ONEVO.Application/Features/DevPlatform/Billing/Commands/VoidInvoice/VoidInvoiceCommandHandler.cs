...
public sealed class VoidInvoiceCommandHandler
    : IRequestHandler<VoidInvoiceCommand, Result<InvoiceDetailDto>>
{
    ...
    public async Task<Result<InvoiceDetailDto>> Handle(VoidInvoiceCommand request, CancellationToken ct)
    {
        ...
        if (!InvoiceStatusRules.CanVoid(invoice.Status))

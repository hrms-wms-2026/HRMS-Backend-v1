...
public sealed class EfFileUploadReservationRepository : IFileUploadReservationRepository
{
    ...
    {
        // NpgsqlRetryingExecutionStrategy forbids a bare BeginTransactionAsync -
        // the whole unit of work (including the initial SaveChangesAsync) must
        // run inside the execution strategy so a transient failure retries the
        // entire block atomically instead of throwing InvalidOperationException.

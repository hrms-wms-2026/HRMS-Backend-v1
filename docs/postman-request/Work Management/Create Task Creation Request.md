...
`categoryId` must be the id of a `TaskCategory` row belonging to this Objective's Project. `priority` is one of `low`, `medium`, `high`, `critical`. `estimatedHours`, `dueDate`, `description`, `storyPoints`, and `sprintId` are optional. Omit `sprintId` (or send `null`) to request a direct task under the Objective with no Sprint.
...
| `404` | Objective, Category, or (when provided) Sprint not found / inactive |

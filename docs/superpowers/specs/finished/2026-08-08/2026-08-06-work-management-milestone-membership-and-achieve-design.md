...
## 11. Addendum (2026-08-07) — Default Objective creator validation

**Origin:** the user, in a follow-up session, pointed out that milestone (Objective) creation happens in two places — the Default Objective auto-created inside `CreateProjectCommandHandler`, and a normal child Objective created via `POST /objectives` — and asked for one consistent rule across both. Re-reading `CreateProjectCommandHandler.cs` against §3 confirmed a real, previously-unnoticed gap: it checks only `employee is not null` before making the caller the Default Objective's `OwnerId` and inserting their `ProjectMember` row; it does not check `EmploymentStatusId == EmploymentStatusIds.Active`, the same rule §3 imposes on every other Head/member assignment.

**Resolution, confirmed with the user:**
- **No change to member-adding.** The Default Objective's creator-becomes-head step needs no "add members" capability — that's what the separate Add/Objective-Member endpoints (§3, §8 #3/#4) are for, and they only apply to explicitly-created (child) milestones. Project creation stays a single insert: creator = head, no member list.
- **Add the active-employee check to `CreateProjectCommandHandler`, for consistency.** Same rule as §3's validation step 2 (`EmploymentStatusId == EmploymentStatusIds.Active`), reusing the `EmploymentStatusIds` constant this design already introduces (§3, Task 1 of the plan). If the caller's employee record is not active, return the same `403 Forbidden` this handler already uses for "no employee record" (existing convention for this handler's *self*-check style — distinct from the `400` used by §3's checks on an *assignee* who isn't the caller).
- **No `IMilestoneMembershipCoordinator`/auto-grant reuse needed here.** The `ProjectMember` row in `CreateProjectCommandHandler` is a guaranteed-new insert (a brand-new project has no prior membership rows to reactivate), so the coordinator's upsert branching is unnecessary. Auto-grant is also unnecessary: `POST /projects` is already `[RequirePermission("projects:access")]`-gated (verified in `ProjectsController.cs`), so the caller already holds the permission §7's auto-grant exists to backfill for child-Objective heads who may not otherwise be authorized project actors.

This does not reopen any other part of the shipped or queued design — it is a single added validation check in one handler, scoped exactly as described above.

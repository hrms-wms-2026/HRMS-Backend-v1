...
**Correction (found after implementation, 2026-08-18):** `ListVisibleAsync`'s predicate always includes the caller's own employee row (`row.e.Id == ownEmployeeId`) — correct for the general Employees list, since obviously you can view yourself, but wrong here: nobody can offboard themselves, and the caller's own row is not a "coverage" row in any meaningful sense. The handler filters `scope.OwnEmployeeId` out of `ListVisibleAsync`'s result before building the response — see `ListOffboardingOverviewQueryHandler.cs` for the exact fix and `ListOffboardingOverviewQueryHandlerTests.Handle_CallersOwnEmployeeRow_IsExcludedEvenThoughListVisibleAsyncIncludesIt` for the regression test.

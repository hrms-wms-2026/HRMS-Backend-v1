  ...
  **Auth:** Tenant session cookie + CSRF header, `[Authorize(Policy = "TenantPolicy")]` (inherited from the controller), plus `[RequirePermission("projects:access")]` on the action — matching Delete/Transfer/Edit's actual attribute (verified against `ObjectivesController.cs`; the flat `GetObjectiveTree` is the one endpoint in this controller with no `[RequirePermission]`, since it authorizes via project membership instead). The Head check (`OwnerId == callerId`) is enforced in the handler on top of that permission gate, same as Delete/Transfer.

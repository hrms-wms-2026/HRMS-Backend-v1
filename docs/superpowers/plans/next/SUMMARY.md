...
- `2026-09-22-centralized-file-upload-phase1-backend.md` — status: pending, not started. Supersedes the just-implemented Phase 0 avatar fix's signed-R2-URL approach for objective members/People module (discovered mid-session: `GetPreSignedURL` is a pure local computation with no reachability check, and the resulting Cloudflare R2 URLs don't reliably load in this dev browser — only the self-avatar streaming endpoint proven in Phase 0 actually works). 7 tasks: `EntityAssetOwnerTypes.Employee`/`.LegalEntity` + `IEntityAssetAccessPolicy` (owner-type-dispatched read auth, since task/objective/project already have different non-negotiable access rules per `GetTaskFileQueryHandler`), a migration backfilling+dropping `avatar_file_id`/`logo_file_id` into `entity_assets`, generic `POST/DELETE/GET /api/v1/files`, deletion of every old per-feature avatar/logo endpoint (including the one-off `GET /employees/{id}/avatar` built and rejected mid-session), and re-pointing `GetMyProfile`/`EfEmployeeRepository`/`CallerIdentityResolver`/`GetObjectiveMembersQueryHandler` at `entity_assets`. Design: `../../specs/next/2026-09-22-centralized-file-upload-design.md`. Companion: `Hrms--Web-application---front-end---v1/docs/superpowers/plans/next/2026-09-22-centralized-file-upload-phase1-frontend.md`. Part 2 (Project/Objective/Task endpoint consolidation + `monitoring_face_scans` fix) is explicitly out of scope here, deferred to a later plan.
- `2026-09-22-centralized-file-upload-phase0-avatar-fix-backend.md` — status: **implemented 2026-09-22** (commits `350f9140`, `648fed43`, `52a5fe25` on `feature/task-subtasks`). 3 tasks: exposes `AvatarFileId` (Guid?) from `GetMyProfileQuery` in place of the dead always-null `AvatarUrl` field, adds `GetMyAvatarQuery`/Handler mirroring `GetLegalEntityLogoQueryHandler`, and a new `GET /employees/me/avatar` streaming endpoint. All automated steps done, 6/6 tests green; the one manual dev-API check step was explicitly skipped by user decision (custom-domain auth flow makes it costly, automated coverage judged sufficient) — not moved to `finished/` since that step stays unchecked. Design: `../../specs/next/2026-09-22-centralized-file-upload-design.md`. Companion: `Hrms--Web-application---front-end---v1/docs/superpowers/plans/next/2026-09-22-centralized-file-upload-phase0-avatar-fix-frontend.md` (also implemented same session). Phase 1 (generic FilesController + entity_assets extension to Employee/LegalEntity) and Phase 2 (frontend shared upload component) are separate, not-yet-written plans per the design's phased delivery.

...
- `phase1-table-inventory.md` — full column-level definitions for all **250 Phase 1 tables** (243 core + 7 Developer Platform Extensions). See breakdown below. Structural verification against actual EF configurations/migrations is queued as Phase 3.
...
- **2026-08-03 — Work Management (Projects) backend build started.** `phase1-table-inventory.md`'s Foundation + Projects + Objectives section was reworked (17→14 tables) to match locked business decisions from the Work Management spec: removed `workspaces`/`workspace_roles`/`workspace_members` (out of scope) and `objective_participants` (superseded by Objective-scoped `project_members`); added `version_statuses`; reworked `projects`/`project_members`/`project_member_invitations`/`versions`/`release_calendar`/`labels`/`project_categories` columns. `entity_assets` is documented but was confirmed **not yet built in code** — first implementation is happening now, scoped to `owner_type = 'project'` only. `objective_categories`/`key_results`/`okr_check_ins` remain documented but deferred (not part of the current Objectives build). See `plans/` for the Work Management foundation-slice plan once written.
- **2026-08-08 — Milestone Membership/Achieve plan closed a two-plan-old documentation gap.** `phase1-table-inventory.md`'s `objective_change_requests` table (shipped 2026-08-04/05 with the milestone-hierarchy plan) had never been added to this inventory; `objectives.reporting_manager_id` was similarly undocumented since the same date. Both were added now, alongside this session's own schema additions (`projects.is_achieved`/`achieved_at`, `objectives.is_achieved`/`achieved_at`). Foundation + Projects + Objectives moves 14→15 tables; Phase 1 total moves 249→250. Rule 2 was followed for this session's own change; the two-plan-old gap was fixed opportunistically while touching the same section, not as a separate audit.

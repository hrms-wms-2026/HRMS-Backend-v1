...
/// <summary>
/// Shared, one-time-per-class setup for LegalEntitiesIntegrationTests: clones the database, boots
...
/// ran 44 times, once per [Fact]. Reviewed every fact for exact-count/global-state assertions
/// before converting (see PositionsIntegrationTests/DepartmentsIntegrationTests for the pattern
/// that review follows) - unlike DepartmentsIntegrationTests, no fact here asserts an exact count
/// against shared fixture state, so no test logic needed to change, only where its setup runs.
/// </summary>
public sealed class LegalEntitiesIntegrationTestsFixture : IAsyncLifetime
{
    ...
    public TenantSession TenantAManager { get; private set; } = null!;
    public TenantSession TenantARegularEmployee { get; private set; } = null!;
    public Guid TenantAId { get; private set; }
    public Guid TenantAPrimaryLegalEntityId { get; private set; }
    public Guid TenantBPrimaryLegalEntityId { get; private set; }
    ...
    public async Task InitializeAsync()
    {
        ...
        TenantA = await ProvisionAndLoginOwnerAsync("legal-ent-a", "Legal Ent A Co", "owner-a@legal-ent.test");
        TenantB = await ProvisionAndLoginOwnerAsync("legal-ent-b", "Legal Ent B Co", "owner-b@legal-ent.test");

        TenantAPrimaryLegalEntityId = await GetPrimaryLegalEntityIdAsync(TenantA);
        TenantBPrimaryLegalEntityId = await GetPrimaryLegalEntityIdAsync(TenantB);

        TenantAId = await GetTenantIdAsync(TenantA.Host);
        var secondCompany = await CreateCompanyAsync(TenantA, "Fixture Second Co", "FIXSEC1", "REG-FIXSEC1");
        TenantASecondLegalEntityId = secondCompany.Id;

        TenantAManager = await SeedAndLoginEmployeeFixtureUserAsync(
            TenantAId, TenantA.Host, "manager@legal-ent-a.test",
            permissionCodes: ["org:read", "org:manage", "legal_entity:update", "legal_entity:delete"],
            roleName: "Legal Entity Manager", legalEntityId: TenantAPrimaryLegalEntityId, employeeNumber: "FIX-MGR-001");

        TenantARegularEmployee = await SeedAndLoginEmployeeFixtureUserAsync(
            TenantAId, TenantA.Host, "regular@legal-ent-a.test",
            permissionCodes: ["org:read", "org:manage"],
            roleName: "Regular Employee", legalEntityId: TenantASecondLegalEntityId, employeeNumber: "FIX-REG-001");
    }
    ...
    public async Task<(Guid Id, JsonElement Body)> CreateCompanyAsync(
    ...
    public async Task<HttpResponseMessage> DeleteCompanyAsync(TenantSession session, Guid id, string confirmName)
    ...
    public object CreateCompanyBody(string name, string companyCode, string registrationNumber) => new
    {
        name,
        companyCode,
        registrationNumber,
...
public sealed class LegalEntitiesIntegrationTests : IClassFixture<LegalEntitiesIntegrationTestsFixture>
{
    private readonly LegalEntitiesIntegrationTestsFixture _fixture;

    public LegalEntitiesIntegrationTests(LegalEntitiesIntegrationTestsFixture fixture)
    {
        _fixture = fixture;
    }
    ...
    {
        var response = await _fixture.SendAsync(HttpMethod.Get, _fixture.TenantA.Host, "/api/v1/org/legal-entities", body: null);
    ...
    {
        var second = await _fixture.CreateCompanyAsync(_fixture.TenantA, "List Test Extra Co", "LTEC1", "REG-LTEC1");
        var deleteResponse = await _fixture.DeleteCompanyAsync(_fixture.TenantA, second.Id, "List Test Extra Co");
        deleteResponse.StatusCode.Should().Be(HttpStatusCode.NoContent);

        var list = await _fixture.GetJsonAsync(_fixture.TenantA, "/api/v1/org/legal-entities");
        var ids = list.EnumerateArray().Select(i => i.GetProperty("id").GetGuid()).ToList();

        ids.Should().Contain(_fixture.TenantAPrimaryLegalEntityId);
        ids.Should().NotContain(second.Id, "a deactivated company must not appear in the default list view");

        var withInactive = await _fixture.GetJsonAsync(_fixture.TenantA, "/api/v1/org/legal-entities?includeInactive=true");
    ...
    public async Task GetGeneralSettings_OwnCompany_Returns200()
    {
        var response = await _fixture.SendAsync(HttpMethod.Get, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{_fixture.TenantAPrimaryLegalEntityId}/general-settings",
            body: null, cookie: _fixture.TenantA.SessionCookie);
    ...
    {
        var response = await _fixture.SendAsync(HttpMethod.Get, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{_fixture.TenantBPrimaryLegalEntityId}/general-settings",
            body: null, cookie: _fixture.TenantA.SessionCookie);
    ...
    {
        var body = _fixture.CreateCompanyBody("Newly Created Co", "NEWCO1", "REG-NEWCO1");
        var response = await _fixture.SendAsync(HttpMethod.Post, _fixture.TenantA.Host, "/api/v1/org/legal-entities", body,
        ...
        var crossTenantGet = await _fixture.SendAsync(HttpMethod.Get, _fixture.TenantB.Host,
            $"/api/v1/org/legal-entities/{id}/general-settings", body: null, cookie: _fixture.TenantB.SessionCookie);
    ...
    {
        var body = _fixture.CreateCompanyBody("Duplicate Name Co", "DUPN1", "REG-DUPN1");
        var first = await _fixture.SendAsync(HttpMethod.Post, _fixture.TenantA.Host, "/api/v1/org/legal-entities", body,
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
        first.StatusCode.Should().Be(HttpStatusCode.Created);

        var duplicate = await _fixture.SendAsync(HttpMethod.Post, _fixture.TenantA.Host, "/api/v1/org/legal-entities",
            _fixture.CreateCompanyBody("Duplicate Name Co", "DUPN2", "REG-DUPN2"),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
        duplicate.StatusCode.Should().Be(HttpStatusCode.Conflict);

        var sameNameOtherTenant = await _fixture.SendAsync(HttpMethod.Post, _fixture.TenantB.Host, "/api/v1/org/legal-entities",
            _fixture.CreateCompanyBody("Duplicate Name Co", "DUPN3", "REG-DUPN3"),
            cookie: _fixture.TenantB.SessionCookie, csrfToken: _fixture.TenantB.CsrfHeader);
    ...
    {
        var first = await _fixture.SendAsync(HttpMethod.Post, _fixture.TenantA.Host, "/api/v1/org/legal-entities",
            _fixture.CreateCompanyBody("Code Dup Co A", "CODEDUP", "REG-CODEDUPA"),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
        first.StatusCode.Should().Be(HttpStatusCode.Created);

        var duplicate = await _fixture.SendAsync(HttpMethod.Post, _fixture.TenantA.Host, "/api/v1/org/legal-entities",
            _fixture.CreateCompanyBody("Code Dup Co B", "CODEDUP", "REG-CODEDUPB"),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
        duplicate.StatusCode.Should().Be(HttpStatusCode.Conflict);
    }
    ...
    {
        var first = await _fixture.SendAsync(HttpMethod.Post, _fixture.TenantA.Host, "/api/v1/org/legal-entities",
            _fixture.CreateCompanyBody("Reg Dup Co A", "REGDUPA", "REG-REGDUP"),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
        first.StatusCode.Should().Be(HttpStatusCode.Created);

        var duplicate = await _fixture.SendAsync(HttpMethod.Post, _fixture.TenantA.Host, "/api/v1/org/legal-entities",
            _fixture.CreateCompanyBody("Reg Dup Co B", "REGDUPB", "REG-REGDUP"),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
        duplicate.StatusCode.Should().Be(HttpStatusCode.Conflict);
    }
    ...
    {
        var missingCountry = await _fixture.SendAsync(HttpMethod.Post, _fixture.TenantA.Host, "/api/v1/org/legal-entities",
            new
            {
        ...
        var missingCurrency = await _fixture.SendAsync(HttpMethod.Post, _fixture.TenantA.Host, "/api/v1/org/legal-entities",
            new
            {
    ...
    {
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Update Target Co", "UPDT1", "REG-UPDT1");
        ...
        var response = await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            ...
            _fixture.UpdateBody("Update Target Co Updated", "UPDT1", "REG-UPDT1", [5, 1, 3]),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
    ...
    {
        var before = await _fixture.GetJsonAsync(_fixture.TenantA, "/api/v1/org/legal-entities");
        var primaryBefore = before.EnumerateArray().Single(i => i.GetProperty("id").GetGuid() == _fixture.TenantAPrimaryLegalEntityId);
        primaryBefore.GetProperty("isPrimary").GetBoolean().Should().BeTrue();

        var current = await _fixture.SendAsync(HttpMethod.Get, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{_fixture.TenantAPrimaryLegalEntityId}/general-settings",
            body: null, cookie: _fixture.TenantA.SessionCookie);
        ...
        var after = await _fixture.GetJsonAsync(_fixture.TenantA, "/api/v1/org/legal-entities");
        var primaryAfter = after.EnumerateArray().Single(i => i.GetProperty("id").GetGuid() == _fixture.TenantAPrimaryLegalEntityId);
    ...
    {
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Working Days Co", "WDC1", "REG-WDC1");

        var empty = await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{company.Id}/general-settings",
            _fixture.UpdateBody("Working Days Co", "WDC1", "REG-WDC1", []),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
        empty.StatusCode.Should().Be(HttpStatusCode.BadRequest);

        var outOfRange = await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{company.Id}/general-settings",
            _fixture.UpdateBody("Working Days Co", "WDC1", "REG-WDC1", [0, 8]),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
        outOfRange.StatusCode.Should().Be(HttpStatusCode.BadRequest);

        var duplicates = await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{company.Id}/general-settings",
            _fixture.UpdateBody("Working Days Co", "WDC1", "REG-WDC1", [1, 1, 2]),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
    ...
    {
        var response = await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantB.Host,
            $"/api/v1/org/legal-entities/{_fixture.TenantAPrimaryLegalEntityId}/general-settings",
            _fixture.UpdateBody("Should Not Apply", "XXX", "REG-XXX", [1, 2, 3, 4, 5]),
            cookie: _fixture.TenantB.SessionCookie, csrfToken: _fixture.TenantB.CsrfHeader);
    ...
    {
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Work Time Get Co", "WTGET1", "REG-WTGET1");
        await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{company.Id}/general-settings",
            _fixture.UpdateBody("Work Time Get Co", "WTGET1", "REG-WTGET1", [1, 2, 3, 4, 5], "09:00", "17:30"),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
    ...
    {
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Work Time Put Co", "WTPUT1", "REG-WTPUT1");

        var response = await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{company.Id}/general-settings",
            _fixture.UpdateBody("Work Time Put Co", "WTPUT1", "REG-WTPUT1", [1, 2, 3, 4, 5], "09:00", "17:30"),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
    ...
    {
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Work Time Only Start Co", "WTOS1", "REG-WTOS1");

        var response = await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{company.Id}/general-settings",
            _fixture.UpdateBody("Work Time Only Start Co", "WTOS1", "REG-WTOS1", [1, 2, 3, 4, 5], "09:00", null),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
    ...
    {
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Work Time Only End Co", "WTOE1", "REG-WTOE1");

        var response = await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{company.Id}/general-settings",
            _fixture.UpdateBody("Work Time Only End Co", "WTOE1", "REG-WTOE1", [1, 2, 3, 4, 5], null, "17:30"),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
    ...
    {
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Work Time Overnight Co", "WTON1", "REG-WTON1");

        var response = await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{company.Id}/general-settings",
            _fixture.UpdateBody("Work Time Overnight Co", "WTON1", "REG-WTON1", [1, 2, 3, 4, 5], "18:00", "09:00"),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
    ...
    {
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Work Time Break Cover Co", "WTBC1", "REG-WTBC1");

        var response = await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{company.Id}/general-settings",
            _fixture.UpdateBody("Work Time Break Cover Co", "WTBC1", "REG-WTBC1", [1, 2, 3, 4, 5], "09:00", "10:00", breakDurationMinutes: 120),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
    ...
    {
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Break Duration Get Co", "BDGET1", "REG-BDGET1");
        await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{company.Id}/general-settings",
            _fixture.UpdateBody("Break Duration Get Co", "BDGET1", "REG-BDGET1", [1, 2, 3, 4, 5], breakDurationMinutes: 60),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
    ...
    {
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Break Duration Put Co", "BDPUT1", "REG-BDPUT1");

        var response = await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{company.Id}/general-settings",
            _fixture.UpdateBody("Break Duration Put Co", "BDPUT1", "REG-BDPUT1", [1, 2, 3, 4, 5], breakDurationMinutes: 45),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
    ...
    {
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Break Duration Null Co", "BDNULL1", "REG-BDNULL1");
        await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{company.Id}/general-settings",
            _fixture.UpdateBody("Break Duration Null Co", "BDNULL1", "REG-BDNULL1", [1, 2, 3, 4, 5], breakDurationMinutes: 30),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);

        var response = await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{company.Id}/general-settings",
            _fixture.UpdateBody("Break Duration Null Co", "BDNULL1", "REG-BDNULL1", [1, 2, 3, 4, 5], breakDurationMinutes: null),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
    ...
    {
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Break Duration Negative Co", "BDNEG1", "REG-BDNEG1");

        var response = await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{company.Id}/general-settings",
            _fixture.UpdateBody("Break Duration Negative Co", "BDNEG1", "REG-BDNEG1", [1, 2, 3, 4, 5], breakDurationMinutes: -1),
            cookie: _fixture.TenantA.SessionCookie, csrfToken: _fixture.TenantA.CsrfHeader);
    ...
    public async Task Update_DecimalBreakDurationMinutes_Returns400()
    {
        ...
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Break Duration Decimal Co", "BDDEC1", "REG-BDDEC1");
    ...
    {
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Break Duration Independent Co", "BDIND1", "REG-BDIND1");

        var response = await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{company.Id}/general-settings",
            _fixture.UpdateBody("Break Duration Independent Co", "BDIND1", "REG-BDIND1", [1, 2, 3, 4, 5],
    ...
    {
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Delete Target Co", "DELT1", "REG-DELT1");

        var response = await _fixture.DeleteCompanyAsync(_fixture.TenantA, company.Id, "Delete Target Co");
        response.StatusCode.Should().Be(HttpStatusCode.NoContent);

        // Row still exists (GET still resolves it), only its status flips.
        var afterDelete = await _fixture.SendAsync(HttpMethod.Get, _fixture.TenantA.Host,
    ...
    {
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Mismatch Target Co", "MISM1", "REG-MISM1");

        var response = await _fixture.DeleteCompanyAsync(_fixture.TenantA, company.Id, "Wrong Name");
        response.StatusCode.Should().Be(HttpStatusCode.BadRequest);
    }
    ...
    {
        // Own freshly-provisioned tenant, not the shared TenantB - IClassFixture means
        // Create_DuplicateNameInsideTenant_Returns409_ButSameNameInAnotherTenantIsAllowed's side
        // effect (it adds a second company to TenantB) would otherwise leave TenantB with more
        // than one active company by the time this fact runs, and the "last active company"
        // business rule would never trigger.
        var soleTenant = await _fixture.ProvisionAndLoginOwnerAsync(
            "last-active-co", "Last Active Co", "owner-last-active@legal-ent.test");
        var primaryLegalEntityId = await _fixture.GetPrimaryLegalEntityIdAsync(soleTenant);

        var response = await _fixture.DeleteCompanyAsync(soleTenant, primaryLegalEntityId, "Last Active Co");
    ...
    {
        var response = await _fixture.SendAsync(HttpMethod.Delete, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{_fixture.TenantBPrimaryLegalEntityId}",
    ...
    {
        var company = await _fixture.CreateCompanyAsync(_fixture.TenantA, "Logo Remove Co", "LOGOC1", "REG-LOGOC1");
        ...
        var after = await _fixture.SendAsync(HttpMethod.Get, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{company.Id}/general-settings", body: null, cookie: _fixture.TenantA.SessionCookie);
    ...
    public async Task List_Owner_SeesAllActiveCompaniesInTenant()
    {
        ...
        ids.Should().Contain(_fixture.TenantAPrimaryLegalEntityId);
        ids.Should().Contain(_fixture.TenantASecondLegalEntityId);
    }

    [Fact]
    public async Task List_ManagerWithLegalEntityUpdate_SeesAllActiveCompaniesInTenant()
    {
        var response = await _fixture.SendAsync(HttpMethod.Get, _fixture.TenantA.Host, "/api/v1/org/legal-entities",
            body: null, cookie: _fixture.TenantAManager.SessionCookie);
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var json = await ReadJsonAsync(response);
    ...
    {
        var response = await _fixture.SendAsync(HttpMethod.Get, _fixture.TenantA.Host, "/api/v1/org/legal-entities",
            body: null, cookie: _fixture.TenantARegularEmployee.SessionCookie);
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var json = await ReadJsonAsync(response);
        var ids = json.EnumerateArray().Select(i => i.GetProperty("id").GetGuid()).ToList();

        ids.Should().ContainSingle().Which.Should().Be(_fixture.TenantASecondLegalEntityId);
    }

    [Fact]
    public async Task List_RegularEmployee_IncludeInactiveTrue_StillOnlyOwnActiveCompany()
    {
        var response = await _fixture.SendAsync(HttpMethod.Get, _fixture.TenantA.Host, "/api/v1/org/legal-entities?includeInactive=true",
            body: null, cookie: _fixture.TenantARegularEmployee.SessionCookie);
        response.StatusCode.Should().Be(HttpStatusCode.OK);
        var json = await ReadJsonAsync(response);
        var ids = json.EnumerateArray().Select(i => i.GetProperty("id").GetGuid()).ToList();
    ...
    {
        var updateResponse = await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{_fixture.TenantASecondLegalEntityId}/general-settings",
            _fixture.UpdateBody("Should Not Apply", "FIXSEC1", "REG-FIXSEC1", [1, 2, 3, 4, 5]),
            cookie: _fixture.TenantARegularEmployee.SessionCookie, csrfToken: _fixture.TenantARegularEmployee.CsrfHeader);
        updateResponse.StatusCode.Should().Be(HttpStatusCode.Forbidden);

        var deleteResponse = await _fixture.SendAsync(HttpMethod.Delete, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{_fixture.TenantASecondLegalEntityId}",
    ...
    {
        // _fixture.TenantAManager was seeded with legal_entity:update and legal_entity:delete but
        // deliberately not legal_entity:create, so this also proves Create is gated
        // independently from Update/Delete.
        var createResponse = await _fixture.SendAsync(HttpMethod.Post, _fixture.TenantA.Host, "/api/v1/org/legal-entities",
            _fixture.CreateCompanyBody("Manager Create Attempt Co", "MGRCA1", "REG-MGRCA1"),
            cookie: _fixture.TenantAManager.SessionCookie, csrfToken: _fixture.TenantAManager.CsrfHeader);
        createResponse.StatusCode.Should().Be(HttpStatusCode.Forbidden);

        var updateResponse = await _fixture.SendAsync(HttpMethod.Put, _fixture.TenantA.Host,
            $"/api/v1/org/legal-entities/{_fixture.TenantASecondLegalEntityId}/general-settings",
            _fixture.UpdateBody("Fixture Second Co Renamed", "FIXSEC1", "REG-FIXSEC1", [1, 2, 3, 4, 5]),
            cookie: _fixture.TenantAManager.SessionCookie, csrfToken: _fixture.TenantAManager.CsrfHeader);

...
// Fixture setup (Testcontainers, tenant provisioning, session login) is duplicated per-file
// rather than shared - matches this repo's existing convention (see
// LeaveEntitlementsAndBalancesIntegrationTests.cs, LeaveRequestsIntegrationTests.cs), which has
// no shared integration-test base for Leave. Not refactored here - out of this plan's scope.
[Collection(WebApplicationFactoryCollection.Name)]
public class LeaveBalanceAuditEndpointTests : IAsyncLifetime
{
    private const string AdminHost = "admin.localhost";
    private const string FixtureUserPassword = "Password123!";
    private const string OwnerEmail = "owner-a@leave-audit.test";
    ...
    private TenantSession _noRead = null!;
    ...
    public async Task InitializeAsync()
    {
        ...
        _owner = await ProvisionAndLoginOwnerAsync("leave-audit", "Leave Audit Co", OwnerEmail);
        _tenantId = await GetTenantIdAsync(_owner.Host);
        _noRead = await SeedAndLoginFixtureUserAsync(
            _tenantId, _owner.Host, "norw@leave-audit.test", permissionCodes: [], roleName: "No Leave Access");
    }
    ...
    [Fact]
    public async Task List_AsOwner_ReturnsAccrualRowsFromGenerate()
    {
        ...
        generate.StatusCode.Should().Be(HttpStatusCode.OK, await generate.Content.ReadAsStringAsync());

        var audit = await SendAsync(HttpMethod.Get, _owner.Host, $"/api/v1/leave/balance-audit?leaveTypeId={leaveTypeId}",
            body: null,
            cookie: _owner.SessionCookie,
            csrfToken: _owner.CsrfHeader);

        audit.StatusCode.Should().Be(HttpStatusCode.OK);
        var json = await ReadJsonAsync(audit);
        json.EnumerateArray().Should().Contain(x => x.GetProperty("changeType").GetString() == "accrual");
    }

    [Fact]
    public async Task List_WithoutLeaveReadPermission_Returns403()
    {
        var response = await SendAsync(HttpMethod.Get, _owner.Host, "/api/v1/leave/balance-audit",
            body: null,
            cookie: _noRead.SessionCookie,
            csrfToken: _noRead.CsrfHeader);
        ...
        string name, Guid leaveTypeId, Guid legalEntityId, bool confirm, decimal annualEntitlementDays = 20m) => new
    {
        ...
        blackoutPeriods = Array.Empty<object>(),

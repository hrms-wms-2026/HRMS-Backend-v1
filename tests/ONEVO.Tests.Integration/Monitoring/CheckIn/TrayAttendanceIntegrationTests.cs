...
public sealed class TrayAttendanceIntegrationTests : IAsyncLifetime
{
    private readonly PostgreSqlContainer _postgres = new PostgreSqlBuilder("postgres:16-alpine")
        .WithDatabase("onevo_attendance_integration_test")
        .WithUsername("test")
        .WithPassword("test")
        .Build();
    ...
    [Fact]
    public async Task GetAttendanceStatus_WithoutJwt_Returns401()
    {
        using var req = new HttpRequestMessage(HttpMethod.Get, "/api/v1/monitoring/tray/attendance-status");
    ...
    [Fact]
    public async Task ClockIn_WithoutJwt_Returns401()
    {
        using var req = new HttpRequestMessage(HttpMethod.Post, "/api/v1/monitoring/tray/clock-in");
        req.Headers.Host = "localhost";
    ...
    [Fact]
    public async Task ClockOut_WithoutJwt_Returns401()
    {
        using var req = new HttpRequestMessage(HttpMethod.Post, "/api/v1/monitoring/tray/clock-out");
        req.Headers.Host = "localhost";
    ...
    [Fact]
    public async Task GetAttendanceStatus_AuthenticatedWithNoAttendanceRecord_ReturnsIsClockedInFalse()
    {
        var slug = $"att-{Guid.NewGuid():N}"[..20];
        ...
        body.GetProperty("is_clocked_in").GetBoolean().Should().BeFalse();
    }

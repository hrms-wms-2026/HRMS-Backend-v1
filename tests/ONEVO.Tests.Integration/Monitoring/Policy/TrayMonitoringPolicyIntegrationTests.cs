...
/// <summary>
/// Full-stack integration tests for the effective tray monitoring policy endpoint
/// under TrayDeviceScheme. Requires Docker.
/// </summary>
...
public sealed class TrayMonitoringPolicyIntegrationTests : IAsyncLifetime
{
    ...
    [Fact]
    public async Task GetPolicy_WithValidJwt_AndNoToggleRow_Returns200_AllFlagsFalse()
    {
        var slug = $"pol-off-{Guid.NewGuid():N}"[..20];
        var user = await SeedActiveUserAsync(slug, $"{slug}@test.dev", "TestPass1!");
        // No toggle row seeded → safe default false for every capability.
        var jwt = await GetTrayJwtForUserAsync(user, $"fp-{slug}");

        using var req = TrayRequest(HttpMethod.Get, "/api/v1/monitoring/tray/policy", jwt);
        var resp = await _client.SendAsync(req);

        resp.StatusCode.Should().Be(HttpStatusCode.OK, await resp.Content.ReadAsStringAsync());

        var body = await resp.Content.ReadFromJsonAsync<JsonElement>();
        body.GetProperty("activity_signal_enabled").GetBoolean().Should().BeFalse();
        body.GetProperty("app_usage_enabled").GetBoolean().Should().BeFalse();
        body.GetProperty("screenshot_enabled").GetBoolean().Should().BeFalse();
        body.GetProperty("inactivity_screenshot_enabled").GetBoolean().Should().BeFalse();
        body.GetProperty("camera_verification_enabled").GetBoolean().Should().BeFalse();
        body.GetProperty("version").GetString().Should().NotBeNullOrWhiteSpace();
        body.GetProperty("valid_until").GetDateTimeOffset().Should().BeAfter(DateTimeOffset.UtcNow);
    }

    [Fact]
    public async Task GetPolicy_WithActivityScreenshotAndAutoCaptureEnabled_Returns200_InactivityScreenshotTrue()
    {
        var slug = $"pol-on-{Guid.NewGuid():N}"[..20];
        var user = await SeedActiveUserAsync(slug, $"{slug}@test.dev", "TestPass1!");
        await EnableTogglesAsync(user.TenantId, activity: true, screenshot: true, autoScreenshot: true);
        var jwt = await GetTrayJwtForUserAsync(user, $"fp-{slug}");
    ...
    [Fact]
    public async Task GetPolicy_WithScreenshotButNoAutoCapture_Returns200_InactivityScreenshotFalse()
    {
        var slug = $"pol-partial-{Guid.NewGuid():N}"[..20];
        var user = await SeedActiveUserAsync(slug, $"{slug}@test.dev", "TestPass1!");
        await EnableTogglesAsync(user.TenantId, activity: true, screenshot: true, autoScreenshot: false);
        var jwt = await GetTrayJwtForUserAsync(user, $"fp-{slug}");
    ...
    private async Task EnableTogglesAsync(Guid tenantId, bool activity, bool screenshot, bool autoScreenshot)
    {
    ...
    private static HttpRequestMessage TrayRequest(HttpMethod method, string path, string jwt)
    {

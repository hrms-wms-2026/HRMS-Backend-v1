...
public sealed class BulkOnboardingRowValidatorTests
{
    ...
    public async Task ValidateRowAsync_Requires_ReportingManager_Column_When_Position_Target_Is_Pooled()
    {
        ...
        _positionRepositoryMock.Setup(p => p.ListByLegalEntityAsync(_tenantId, It.IsAny<Guid>(), It.IsAny<bool>(), It.IsAny<Guid?>(), It.IsAny<CancellationToken>()))
    ...
    public async Task ValidateRowAsync_Resolves_ReportingManager_By_Email()
    {
        ...
        _positionAssignments.Setup(a => a.GetActiveHoldersAsync(_tenantId, pooledTargetId, It.IsAny<CancellationToken>()))
            .ReturnsAsync(new List<PositionActiveHolder>
            {
                new(chosenManagerId, "A", "One", "a@acme.test", null),
                new(Guid.NewGuid(), "B", "Two", "b@acme.test", null),
            });
        ...
        {
            ["firstName"] = "First Name", ["lastName"] = "Last Name", ["workEmail"] = "Email",
            ["startDate"] = "Start", ["position"] = "Position", ["reportingManager"] = "Reports To",
        };
        var rawData = new Dictionary<string, string>
        {
            ["First Name"] = "Jane", ["Last Name"] = "Doe", ["Email"] = "jane@acme.com",
            ["Start"] = "2026-09-01", ["Position"] = "Junior Engineer", ["Reports To"] = "a@acme.test",
        };

        var outcome = await _validator.ValidateRowAsync(_tenantId, _batch, rawData, mapping, new HashSet<string>(), CancellationToken.None);

        Assert.True(outcome.IsValid);
        Assert.Equal(chosenManagerId, outcome.ReportsToEmployeeId);
    }
    ...
    public async Task ValidateRowAsync_Fails_When_ReportingManager_Email_Not_A_Current_Holder()
    {
        ...
        {
            ["First Name"] = "Jane", ["Last Name"] = "Doe", ["Email"] = "jane@acme.com",
            ["Start"] = "2026-09-01", ["Position"] = "Junior Engineer", ["Reports To"] = "not-a-holder@acme.test",
        };
